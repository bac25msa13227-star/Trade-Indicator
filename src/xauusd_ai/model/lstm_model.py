...
def create_lstm_model(settings) -> LSTMModelWrapper | None:
    """Factory function to create LSTM model from settings."""
    if not TORCH_AVAILABLE:
        logging.warning("PyTorch not available - skipping LSTM model")
        return None
        
    lstm_config = getattr(settings.training, 'lstm', None)
    ...
    # Use config values
    return LSTMModelWrapper(
        sequence_length=getattr(lstm_config, 'sequence_length', 20),
        hidden_size=getattr(lstm_config, 'hidden_size', 64),
        num_layers=getattr(lstm_config, 'num_layers', 2),
        dropout=getattr(lstm_config, 'dropout', 0.2),
        learning_rate=getattr(lstm_config, 'learning_rate', 0.001),
        epochs=getattr(lstm_config, 'epochs', 50),
        batch_size=getattr(lstm_config, 'batch_size', 64),
    )
